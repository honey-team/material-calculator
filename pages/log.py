...
def log(page: ft.Page):
    def get_log(a, b): return loga(b, a)
    def get_log10(b): return loga(b, 10)
    def get_ln(b): return loga(b)
    ...
    def change_args(*_):
        try:
            log.value = LOG.format(a=get_low(Number(a.value)), b=Number(b.value), x=get_log(float(Number(a.value)), float(Number(b.value))))
    ...
    m = mload()
    a = ft.TextField(value=m['pages']['log']['a'], label='Основание логарифма', on_change=change_args, input_filter=INPUT_FILTER)
    b = ft.TextField(value=m['pages']['log']['b'], label='Число логарифма', on_change=change_args, input_filter=INPUT_FILTER)
